...
impl Crowdfund {
    ...
    pub fn deposit(e: Env, from: Address, amount: i128) {
        ...
        // INTER-CONTRACT CALL: Call Custom Token Contract to transfer funds
        let token_client = token::Client::new(&e, &token_addr);
        // Transfer from contributor to crowdfund contract
    ...
    pub fn refund(e: Env, to: Address) {
        // We simulate "expired" state by simply allowing refund if state is running or expired
        // But let's just create an `expire` function and require expiration
        let state: State = e.storage().instance().get(&DataKey::State).unwrap();
